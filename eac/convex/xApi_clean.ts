...
export const authenticateX = action({
  ...
  handler: async (ctx, args): Promise<XApiResponse> => {
    console.log('🔍 authenticateX: Starting OAuth flow', {
      connectionId: args.connectionId,
      hasCode: !!args.code,
      hasCodeVerifier: !!args.codeVerifier,
      codeLength: args.code?.length,
      verifierLength: args.codeVerifier?.length
    });
    ...
    try {
      ...
      console.log('🔍 authenticateX: Connection query result', {
        found: !!connection,
      ...
      if (!connection) {
        console.error('❌ authenticateX: Connection not found');
        return {
          success: false,
