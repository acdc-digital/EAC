...
exports.EACSocialWorkflowOptimizer = void 0;
class EACSocialWorkflowOptimizer {
    projectRoot;
    ...
    generateOptimization(workflow, focus) {
...
### Current Steps
${current.steps.map((step, i) => `${i + 1}. **${step.name}** (${step.estimatedTime}min) - ${step.automation} automation
...
### Optimized Steps
${optimized.steps.map((step, i) => `${i + 1}. **${step.name}** (${step.estimatedTime}min) - ${step.automation} automation
   ${step.description}${step.status === 'pending' ? ' 🆕' : ''}`).join('\n')}
...
exports.EACSocialWorkflowOptimizer = EACSocialWorkflowOptimizer;
//# sourceMappingURL=social-workflow-optimizer.js.map